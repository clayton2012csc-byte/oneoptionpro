...
-- Único passo antes de executar: troque SEGREDO_CRON_A_CADASTRAR pelo valor da chave CRON_SECRET
...
SELECT cron.schedule('oneoption-ai-round-morning', '0 10 * * *', $$
  SELECT net.http_post(url := 'https://oneoptionpro.lovable.app/api/public/ai/round?slot=morning', headers := '{"Content-Type":"application/json","x-cron-secret":"SEGREDO_CRON_A_CADASTRAR"}'::jsonb, body := '{}'::jsonb);
$$);
SELECT cron.schedule('oneoption-ai-round-afternoon', '0 16 * * *', $$
  SELECT net.http_post(url := 'https://oneoptionpro.lovable.app/api/public/ai/round?slot=afternoon', headers := '{"Content-Type":"application/json","x-cron-secret":"SEGREDO_CRON_A_CADASTRAR"}'::jsonb, body := '{}'::jsonb);
$$);
SELECT cron.schedule('oneoption-ai-round-night', '0 21 * * *', $$
  SELECT net.http_post(url := 'https://oneoptionpro.lovable.app/api/public/ai/round?slot=night', headers := '{"Content-Type":"application/json","x-cron-secret":"SEGREDO_CRON_A_CADASTRAR"}'::jsonb, body := '{}'::jsonb);
$$);
